...
// "ACC System" placeholder for empty user fields, matches old MissingUser.
function userFormatter(cell) {
  const v = cell.getValue();
  if (v === undefined || v === null || v === "") return "ACC System";
  return escapeHtml(v);
}
...
function buildMidpColumns() {
  ...
  // MIDP and the Drawing Register variants share the same ACC custom-
  ...
  const cols = [
    ...
    { title: "Revision", field: "revision", width: 90, formatter: revisionFormatter, editor: "input", editable, cellEdited, headerFilter: "input" },
    ...
    { title: "File Description", field: "file_description", widthGrow: 2, minWidth: 160, formatter: fileDescriptionFormatter, editor: "input", editable, cellEdited, headerFilter: "input" },
    { title: "Title Line 1", field: "title_line_1", widthGrow: 3, minWidth: 200, formatter: missingFormatter, editor: "input", editable, cellEdited, headerFilter: "input" },
    { title: "Title Line 2", field: "title_line_2", widthGrow: 2, minWidth: 140, formatter: missingFormatterYellow, editor: "input", editable, cellEdited, visible: !defaultHiddenColumns.includes("Title Line 2") },
    { title: "Title Line 3", field: "title_line_3", widthGrow: 2, minWidth: 140, formatter: missingFormatterYellow, editor: "input", editable, cellEdited, visible: !defaultHiddenColumns.includes("Title Line 3") },
    { title: "Title Line 4", field: "title_line_4", widthGrow: 2, minWidth: 140, formatter: missingFormatterYellow, editor: "input", editable, cellEdited, visible: !defaultHiddenColumns.includes("Title Line 4") },
    { title: "Status", field: "status", width: 90, formatter: missingFormatter, editor: "input", editable, cellEdited, headerFilter: "list", headerFilterParams: { valuesLookup: "all", clearable: true } },
    { title: "Activity Code", field: "activity_code", widthGrow: 1, minWidth: 120, formatter: missingFormatterYellow, editor: "input", editable, cellEdited, visible: !defaultHiddenColumns.includes("Activity Code") },
  ];

  if (isA66) {
    cols.push({ title: "Series", field: "series", width: 90, formatter: missingFormatterYellow, editor: "input", editable, cellEdited });
  }
...
// Generic cell-edited factory. Each tab uses a different ACC custom-
...
function makeCellEditedHandler(getMapping) {
  return async (cell) => {
    ...
    if (!colDef) {
      console.warn(`No ACC attribute mapping for field "${fieldName}" — skipping PATCH`);
      if (typeof showPopup === "function") {
        showPopup("Update skipped", `No ACC attribute mapping for &quot;${fieldLabel}&quot;`);
      }
      return;
    }
    ...
    try {
      ...
      if (result && result.ok) {
        ...
        showPopup("Update failed" + status, `${file.name || "File"} — ACC rejected the change to ${fieldLabel}`);
      }
    } catch (err) {
      if (typeof showPopup === "function") {
...
async function tabulatorToggleEdit() {
  if (isClient) return;
  // The first time edit mode is enabled we need the ACC custom-attribute
